...
- [ ] Tabla `producto_lotes`:
  ...
  CREATE INDEX idx_lotes_caducidad ON producto_lotes(negocio_id, fecha_caducidad);
  ```
- [ ] `ALTER TABLE productos ADD COLUMN IF NOT EXISTS controla_lote BOOLEAN DEFAULT false;` — solo medicamentos lo activan.
- [ ] `entradasController`: si `controla_lote`, requiere `lote` y `fecha_caducidad`, sumar al lote o crear nuevo.
- [ ] `ventasController.crear`: si `controla_lote`, descontar usando **FEFO** (First Expired, First Out) — ordenar lotes por `fecha_caducidad ASC` y descontar.
- [ ] `venta_items` agregar columna `lote_id INT REFERENCES producto_lotes(id)`.
- [ ] Reporte admin: productos próximos a caducar (30/60/90 días) y vencidos.
- [ ] Bloquear venta de lote ya vencido (configurable por negocio).
